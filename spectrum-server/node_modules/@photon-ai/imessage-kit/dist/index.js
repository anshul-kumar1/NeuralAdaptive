...
import { Unarchiver, NSAttributedString } from '@parseaple/typedstream';
import { createRequire } from 'module';
import { existsSync, readdirSync, lstatSync, unlinkSync, watch, writeFileSync, statSync } from 'fs';
...
function getDarwinMajorVersion(osRelease = release()) {
...
async function fetchAndSave(url, timeout, maxSize, debug, signal) {
  ...
  if (contentType.includes("avif") || contentType.includes("webp")) {
    ...
    const tempPath = join(TEMP_DIR, generateSecureFilename(ext2));
    writeFileSync(tempPath, buffer, { flag: "wx", mode: 384 });
    let converted;
    try {
  ...
  writeFileSync(path, buffer, { flag: "wx", mode: 384 });
  return path;
}
...
var MessageSender = class {
  ...
  createTrackingPromise(chatId, text, hasText, paths, sentAt) {
    ...
    if (paths.length > 0) {
      messagePromise = new MessagePromise({
        chatId,
        text,
        attachmentName: paths[0] ? basename(paths[0]) : void 0,
        isAttachment: true,
        sentAt
      });
    } else if (hasText) {
  ...
  async resolveAttachment(path, signal) {
    if (isURL(path)) {
...
var TempFileManager = class {
  ...
  getStats() {
    let currentFiles = 0;
    try {
      if (existsSync(TEMP_DIR2)) {
        const files = readdirSync(TEMP_DIR2);
  ...
  async removeFiles(options) {
    ...
    const now = Date.now();
    try {
      if (!existsSync(TEMP_DIR2)) return { removed, errors };
      const files = readdirSync(TEMP_DIR2);
      for (const file of files) {
        if (!file.startsWith(TEMP_FILE_PREFIX)) continue;
        const filePath = join(TEMP_DIR2, file);
        try {
